// NOTE:
// To select the optimal resistor value for your specific photoresistor, you should consult the datasheet of the photoresistor for its resistance range under different light conditions. Choose a fixed resistor value that matches the photoresistor's resistance at the midpoint of the expected light levels, or you can experiment with different fixed resistor values to find the one that provides the best performance in your specific application.

// Keep in mind that the ADC resolution and the input voltage also play a role in determining the accuracy and sensitivity of your measurements. With a higher ADC resolution or a lower input voltage, you can achieve more accurate and sensitive measurements, which may allow you to use a wider range of fixed resistor values.
